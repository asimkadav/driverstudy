...
let ttdcum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
let dmacum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
let portmmcum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
let buscum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
let ttkcum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
let synccum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
let alloccum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
let kdevcum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
let klibcum_calls: (string, int) Hashtbl.t = (Hashtbl.create 15);;
...
class driverVisitor = object (self) (* self is equivalent to this in Java/C++ *)
 ...
 (* Finds all the lvals (string) in a CIL expression (hopefully). 
  ...
  method traversecallers2 (a:callnode)(call_depth: int) :int = 
    begin
        ...
        let recurseCg  _ (cl:callnode):unit =
         ...
         (* Propogate tags *)
              ...
              let cl_name = (nodeName cl.cnInfo) in
                ...
                if (self#is_mmio fn_name == 1) && (self#is_mmio cl_name !=1) then (
                ...
                if (self#is_bus fn_name == 1) && (self#is_bus cl_name !=1)  then    (
                ...
                if (self#is_inhash khelper_fns fn_name == 1) && (self#is_inhash khelper_fns cl_name !=1) then (
                ...
                if (self#is_inhash kdev_fns fn_name == 1)  && (self#is_inhash kdev_fns cl_name !=1)  then (
                  Hashtbl.add kdev_fns cl_name fn_name;
				  Hashtbl.replace kdev_calls cl_name (self#kdev_count cl_name + self#kdev_count fn_name);
				);

                if (self#is_inhash devreg_fns fn_name == 1)  && (self#is_inhash devreg_fns cl_name !=1)  then (
                  Hashtbl.add devreg_fns cl_name fn_name;
				  Hashtbl.replace kdev_calls cl_name (self#kdev_count cl_name + self#kdev_count fn_name);
				);

                if (self#is_inhash time_fns fn_name == 1)  && (self#is_inhash time_fns cl_name !=1)  then (
 ...
 *)
 ...
 method checkforpair (fn_name:string)(cl_name:string):unit =
 begin
	let found = ref 0 in	
	Printf.fprintf stderr "In check for pair.\n";
    try 
	  let sl:string list = Hashtbl.find_all seencnidstring fn_name in
		for s = 0 to (List.length sl) - 1 do
		 let ret_str = (List.nth sl s) in
		if (String.compare ret_str cl_name == 0) then (
		 	found := 1;
			Printf.fprintf stderr "Already %s %s %s\n" fn_name cl_name ret_str ; )
	   done;
	   if (!found == 0) then
		Hashtbl.add seencnidstring fn_name cl_name;
	with Not_found -> (Hashtbl.add seencnidstring fn_name cl_name);
 end

  method populate_callers (fn_name:string)(cl_name:string) :unit =
  begin
	 Printf.fprintf stderr "In populate callers for %s %s.\n" fn_name cl_name;  
	 let sl:string list =  Hashtbl.find_all seencnidstring cl_name in
	 for s = 0 to (List.length sl) - 1 do
	   let cur_str = (List.nth sl s) in
		self#checkforpair fn_name cur_str;   
		
	 done;

  end 

  method populate_counts (fn_name:string)(cl_name:string) :unit =
  begin
	Printf.fprintf stderr "In populate counts.. old:%s %d| new: %s %d +\n" cl_name (self#alloc_count cl_name) fn_name (self#alloc_count fn_name) ;
	Hashtbl.replace alloc_calls cl_name (self#alloc_count cl_name + self#alloc_count fn_name);
  end
      ...
      if (IH.length a.cnCallers > 0) then  (
        ...
        let recurseCg  _ (cl:callnode):unit =
         ...
         (* Propogate tags *)
              let fn_name = (nodeName a.cnInfo) in
              let cl_name = (nodeName cl.cnInfo) in
				Printf.fprintf stderr "check %s %s : \n" fn_name cl_name;
				 (self#checkforpair fn_name cl_name );
                (* Propogate tags for ttd and ttk in upward direction. *)
                (* For counts also propogate count totals *)

                if (self#ttd fn_name == 1)  then (
                  Hashtbl.add ttd_fns cl_name fn_name;
                  Hashtbl.replace ttdcum_calls cl_name (self#ttd_count cl_name + self#ttd_count fn_name);  
                );

                if (self#is_dma fn_name == 1)  then	(
                  Hashtbl.add dma_fns cl_name fn_name;
                  Hashtbl.replace dmacum_calls cl_name (self#dma_count cl_name + self#dma_count fn_name); 
				);


                if (self#is_port fn_name == 1)  then (
                  Hashtbl.add port_fns cl_name fn_name;
                  Hashtbl.replace portmmcum_calls cl_name (self#portmm_count cl_name + self#portmm_count fn_name); 
				);
                ...
                if (self#ttk fn_name == 1)  then (
                  Hashtbl.add ttk_fns cl_name fn_name;
                  Hashtbl.replace ttkcum_calls cl_name (self#ttk_count cl_name + self#ttk_count fn_name); 
                   
				);
                
                if (self#has_sync fn_name == 1) then  (
                  Hashtbl.add sync_fns cl_name fn_name;
                  Hashtbl.replace synccum_calls cl_name (self#sync_count cl_name + self#sync_count fn_name); 
                );
                ...
                if (self#is_bus fn_name == 1) then    (
                  Hashtbl.add bus_fns cl_name fn_name;
                  Hashtbl.replace buscum_calls cl_name (self#bus_count cl_name + self#bus_count fn_name); 
				);

                if (self#has_thread fn_name == 1) then
                  Hashtbl.add thread_fns cl_name fn_name;
                
                if (self#is_inhash khelper_fns fn_name == 1) then (
                  Hashtbl.add khelper_fns cl_name fn_name;
                  Hashtbl.replace klibcum_calls cl_name (self#klib_count cl_name + self#klib_count fn_name); 
				);

                if (self#is_inhash kdev_fns fn_name == 1)  then (
                  Hashtbl.add kdev_fns cl_name fn_name;
				  Hashtbl.replace kdevcum_calls cl_name (self#kdev_count cl_name + self#kdev_count fn_name);
				);

                if (self#is_inhash devreg_fns fn_name == 1)  then (
                  Hashtbl.add devreg_fns cl_name fn_name;
				  Hashtbl.replace kdevcum_calls cl_name (self#kdev_count cl_name + self#kdev_count fn_name);
				);

                if (self#is_inhash time_fns fn_name == 1)   then (
                  Hashtbl.add time_fns cl_name fn_name;
				);

                if (self#is_allocator fn_name == 1) && (self#is_allocator cl_name !=1) then  (
                  Hashtbl.add allocator_fns cl_name fn_name;
                  (* Printf.fprintf stderr "alloc counts %s(%d) and %s(%d)" cl_name (self#alloc_count cl_name) fn_name (self#alloc_count fn_name); *) 
				  Hashtbl.replace alloc_calls cl_name (self#alloc_count cl_name + self#alloc_count fn_name);
 				  (* Printf.fprintf stderr "alloc:%s called by %s(%d)\n" fn_name cl_name (self#hash_count alloccum_calls cl_name); *) 
                );
(*
                if (self#is_process fn_name = 1) then
                  Hashtbl.add process_fns cl_name fn_name;
...
(* Comment below to Disable propogation for call countsi *) 
		...
		Hashtbl.iter self#populate_callers seencnidstring; 
 		Printf.fprintf stderr "SEENCNID START.\n";
		Hashtbl.iter self#printfnname seencnidstring;
		
 		Printf.fprintf stderr "SEENCNID STOP.\n";
		Hashtbl.clear seencnids;
        Hashtbl.clear seencnidsprod;
 ...
 *)
        if (self#ttk_count b) < (self#klib_count b) then (
   	                 Printf.fprintf stderr "Analysis exception.\n";
        ...
        fn_len_data2 := !fn_len_data2^Printf.sprintf "\n\nfn:%s loc:%d ioctl:%d init:%d toplevel:%d cleanup:%d pm:%d err:%d config:%d proc:%d modpm:%d devctl:%d ttd:%d ttk:%d alloc:%d core:%d sync:%d process:%d event:%d thread:%d dma:%d bus:%d ttdc:%d syncc:%d allocc:%d ttkc:%d port:%d  mmio:%d intr:%d khelp:%d kdev:%d devreg:%d time:%d dmac%d busc%d portmmc %d kdevc %d klibc %d" b a
         (self#is_ioctl b) (self#is_init b) (self#toplevel b)  (self#is_cleanup b) (self#is_pm b) (self#is_err b) (self#is_config b) (self#is_proc b) (self#is_modpm b) (self#is_devctl b) (self#ttd b) (self#ttk b) (self#is_allocator b) (self#is_core b) (self#has_sync b) (self#is_process b) (self#has_event b) (self#has_thread b) (self#is_dma b) (self#is_bus b) (self#ttd_count b) (self#sync_count b) (self#alloc_count b) (self#hash_count ttkcum_calls b) (self#is_port b) (self#is_mmio b) (self#is_intr b) (self#is_inhash khelper_fns b) (self#is_inhash kdev_fns b) (self#is_inhash devreg_fns b) (self#is_inhash time_fns b) (self#dma_count b) (self#bus_count b) (self#portmm_count b) (self#kdev_count b) (self#klib_count b);  
        fn_len_data := !fn_len_data^Printf.sprintf "%s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d " b a
         (self#is_ioctl b) (self#is_init b) (self#toplevel b)  (self#is_cleanup b) (self#is_pm b) (self#is_err b) (self#is_config b) (self#is_proc b) (self#is_modpm b) (self#is_devctl b) (self#ttd b) (self#ttk b) (self#is_allocator b) (self#is_core b) (self#has_sync b) (self#is_process b) (self#has_event b) (self#has_thread b) (self#is_dma b) (self#is_bus b) (self#ttd_count b) (self#sync_count b) (self#hash_count alloccum_calls b) (self#ttk_count b) (self#is_port b) (self#is_mmio b) (self#is_intr b) (self#is_inhash khelper_fns b) (self#is_inhash kdev_fns b) (self#is_inhash devreg_fns b) (self#is_inhash time_fns b) (self#dma_count b) (self#bus_count b) (self#portmm_count b) (self#kdev_count b) (self#klib_count b);  
     end
   end
