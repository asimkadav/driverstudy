   ...
   begin
     ...
     match s.skind with 
       | Instr(ilist) -> ( (* Hashtbl.add stmt_cluster !cur_l 1; *)
                           for j = 0 to (List.length ilist) - 1 do
                             let cur_instr = (List.nth ilist j) in
                               match cur_instr with   
                                    | Call(lvalue_option,e,el,loc) -> (Hashtbl.add stmt_cluster loc.line 2;);
                       ...
                       DoChildren;);
       | Return(_, loc) -> Hashtbl.add stmt_cluster !cur_l 3; DoChildren;
       | Goto (_, loc) -> Hashtbl.add stmt_cluster !cur_l 5; DoChildren;
       | Break (loc) -> Hashtbl.add stmt_cluster !cur_l 4; DoChildren;
       | Continue (loc) -> Hashtbl.add stmt_cluster !cur_l 4; DoChildren;
       | If (_, _, _, loc) -> Hashtbl.add stmt_cluster !cur_l 6; DoChildren;
       | Switch (_, _, _, loc) -> Hashtbl.add stmt_cluster !cur_l 7 ; DoChildren;
       | Loop (_,loc, _, _) -> Hashtbl.add stmt_cluster !cur_l 4; DoChildren;
       | Block (_) -> Hashtbl.add stmt_cluster !cur_l 1; DoChildren;
       | TryFinally (_, _, loc) -> Hashtbl.add stmt_cluster !cur_l 8; DoChildren;
       | TryExcept (_, _, _, loc) -> Hashtbl.add stmt_cluster  !cur_l 9; DoChildren;                   
       | _ ->DoChildren; 
