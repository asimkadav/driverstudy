...
let stmt_cluster : (int*int, int) Hashtbl.t = (Hashtbl.create 100);;
...
class driverVisitor = object (self) (* self is equivalent to this in Java/C++ *)
  ...
  val mutable fname = ref "";
  val mutable z_data = ref "";   
    val mutable cnt = ref 0;                     
  val mutable nd = ref 0; (* nexting depth *) 
 ...
 (* Finds all the lvals (string) in a CIL expression (hopefully). 
  ...
  method printfndata (b,c: int*int) (n:int):unit = 
    begin
      if (b > 0) then
        fn_data := !fn_data^Printf.sprintf "%d %d " b c; 
    end

  method genxdata (b,c: int*int) (n:int):unit = 
    begin
      if (b > 0) then
        x_data := !x_data^Printf.sprintf " %d," b;
    end

  method genydata (b,c: int*int) (n:int):unit = 
    begin 
      if (b > 0) then
        y_data := !y_data^Printf.sprintf " %d," c; 
    end
      
  method genzdata (b,c: int*int) (n:int):unit =
    begin
      if (b > 0) then
        z_data := !z_data^Printf.sprintf " %d," n;
    end

      

  method gengraphdata (b,c:int*int) (n:int) :unit =
    begin
      if (b >0) then
   ...
   begin
      match ins with  
        | Call(lvalue_option,e,el,loc) -> 
                if(List.mem (exp_to_string e) device_fns=true) then 
                  (Hashtbl.add stmt_cluster (loc.line,17) !nd;) 
                else (Hashtbl.add stmt_cluster (loc.line,12) !nd); 
                       DoChildren;
        | Set (lv, ex, loc) ->Hashtbl.add stmt_cluster (loc.line,2) !nd; DoChildren;
        |_ -> DoChildren;
   end
                     ...
                     (*      for j = 0 to (List.length ilist) - 1 do
                             let cur_instr = (List.nth ilist j) in
                               match cur_instr with   
                                    | Call(lvalue_option,e,el,loc) -> if(List.mem (exp_to_string e) device_fns=true) then (Hashtbl.add stmt_cluster (loc.line,17) !nd; 
                                                 ) else (Hashtbl.add stmt_cluster (loc.line,12) !nd);
                                    | Set (lv, ex, loc) ->(Hashtbl.add stmt_cluster (loc.line,2) !nd;);
                                    |_ -> ();
                                                                      
                           done; 
                       *)        
                       DoChildren;);
       | Return(_, loc) -> (Hashtbl.add stmt_cluster (!cur_l,60) !nd; Hashtbl.add stmt_cluster (!cur_l, 60) !nd; Hashtbl.add stmt_cluster (!cur_l,60) !nd;   DoChildren;);
       | Goto (_, loc) -> Hashtbl.add stmt_cluster (!cur_l, 50) !nd; DoChildren;
       | Break (loc) -> Hashtbl.add stmt_cluster (!cur_l, 45) !nd; DoChildren;
       | Continue (loc) -> Hashtbl.add stmt_cluster (!cur_l, 70) !nd; DoChildren;
       | If (_, _, _, loc) -> Hashtbl.add stmt_cluster (!cur_l, 80) (!nd+1); DoChildren;
       | Switch (_, _, _, loc) -> Hashtbl.add stmt_cluster (!cur_l, 100) (!nd+1) ; DoChildren;
       | Loop (_,loc, _, _) ->  Hashtbl.add stmt_cluster (!cur_l, 40) (!nd+1);  DoChildren;  
       | Block (_) -> (Hashtbl.add stmt_cluster (!cur_l, 6) !nd; Hashtbl.add stmt_cluster (!cur_l ,6) !nd;   DoChildren;);
       | TryFinally (_, _, loc) -> Hashtbl.add stmt_cluster (!cur_l, 110) !nd; DoChildren;
       | TryExcept (_, _, _, loc) -> Hashtbl.add stmt_cluster  (!cur_l, 120) !nd; DoChildren;                   
       | _ ->DoChildren; 
   end
   ...
   begin
     ...
     Hashtbl.iter self#genydata stmt_cluster;
      Hashtbl.iter self#genzdata stmt_cluster;
     ...
     let z_len = (String.length !z_data) in
       if (z_len  > 0) then 
                !z_data. [z_len - 1] <- ' ';
     ...
     let func_str = ref "" in
     func_str := Printf.sprintf "%s_%s" curr_func.svar.vname !fnames;  
     Printf.fprintf stderr "%s = [%s ; %s; %s]; \n" !func_str !x_data !y_data !z_data;
