 ...
 let match_regexp2 = regexp (".*"^"dev_dbg"^".*") in
	...
	*)
    ...
    | Instr (ilist) ->
         ...
         match cur_i with
              | Call (_,e, el, loc) -> (Printf.fprintf stderr "lckpk %s" (exp_to_string e); found:= !found + self#loceprintk e;if (((String.compare (exp_to_string e) "printk") == 0) 
...
(*    b.bstmts <- [logg_stmt]@b.bstmts ; *)
  ...
  method get_stmt_from_if_stmt_deref (cont_deref_lvals: lval list) (labels: label list) : stmt =
   ...
   begin
	...
	Printf.fprintf stderr "memory bugs %d unnecssary panics %d ret %d rtc %d pk %d pkrtc %d." mem_deref_bugs !halt_count return_on_device_error report_timeout_counter ret_pk_count pk_in_rtc; (* num_bad_ptr_lvals; *)
