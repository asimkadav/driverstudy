  ...
  method gengraphdata (b:int) (c:int) :unit =
   ...
   begin
     ...
     match s.skind with 
                           ...
                           for j = 0 to (List.length ilist) - 1 do
                             let cur_instr = (List.nth ilist j) in
                               match cur_instr with   
                                    | Call(lvalue_option,e,el,loc) -> if(List.mem (exp_to_string e) device_fns=true) then (Hashtbl.add stmt_cluster loc.line 17; 
                                                 ) else (Hashtbl.add stmt_cluster loc.line 12);
                                    | Set (lv, ex, loc) ->(Hashtbl.add stmt_cluster loc.line 2;);
                                    |_ -> ();
       ...
       | Block (_) -> (Hashtbl.add stmt_cluster !cur_l 6; Hashtbl.add stmt_cluster !cur_l 6;   DoChildren;);
