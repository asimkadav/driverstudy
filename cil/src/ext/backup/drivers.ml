                             ...
                             if (self#isbad_exp e  = 1) then
                                     begin
                                         ...
                                         temp_bad_functions <-
                                             vi.vname::temp_bad_functions;
					 Hashtbl.add when_dirrrty (vi.vname, curr_func.svar.vname) loc.line;
                                         (* Printf.fprintf stderr "Adding TO BAD:
                                             %s.\n" vi.vname; *)
                                     end;
   ...
   begin
     match e with
       ...
       | AddrOf(lv_inner) -> 
        ...
        | BinOp (b, e1, e2, typ) -> (Printf.fprintf stderr "and op in array.\n"; if ((b != LAnd) & (b != BAnd) ) then 
    ...
    match i with
        | Call(lval_option,exp,e_list,l) ->
	     ...
	     last_array_device_call_loc <- -1;
             if (isbad e1 [] == 1) then      (
               Hashtbl.clear hist_array_dirty;
  		          ...
  		          for strctr = 0 to (List.length str_list) -1 do
                           let cur_str = (List.nth str_list  strctr) in
                            if (String.compare (cur_str) "jiffies" = 0)
                                  then (rc_list := "__nooks_timer"::!rc_list;);
   		              if ((String.compare (cur_str) "time_after_eq" = 0) ||
                                (String.compare (cur_str) "timeout" = 0) ||
...
(*	    | Return (_,_) -> rc_list := "__nooks_timer"::!rc_list; *)
   ...
   method loceprintk (e:exp) : int =
				   begin
			   let found = ref 0 in
 let match_regexp = regexp (".*"^"DPRINT"^".*") in
 let match_regexp1 = regexp (".*"^"dev_err"^".*") in
 let match_regexp2 = regexp (".*"^"dev_dbg"^".*") in
				   if ((( Str.string_match match_regexp (exp_to_string e) 0) == true)
				   || (( Str.string_match match_regexp2 (exp_to_string e) 0) == true)
				   || (( Str.string_match match_regexp1 (exp_to_string e) 0) == true)
				   || ((String.compare (exp_to_string e) "dev_warn") = 0))
							   then found:= 1 ; 
	...
	*)
    ...
    | Instr (ilist) ->
         ...
         match cur_i with
              | Call (_,e, el, loc) -> (Printf.fprintf stderr "lckpk %s" (exp_to_string e); found:= !found + self#loceprintk e;if (((String.compare (exp_to_string e) "DPRINT") == 0) 
		|| ((String.compare (exp_to_string e) "dev_err") == 0) 
		|| ((String.compare (exp_to_string e) "dev_warn") == 0) 
		|| ((String.compare (exp_to_string e) "dev_dbg") == 0)) 


		then found:= 1 ; );
	      | _ -> ();
	done;
     | If(e,b1,b2,l) -> found := (self#loceprintk e)+ (self#locateprintk b1) +  (self#locateprintk b2); 
     | Loop (b1,l,_,_) -> found := self#locateprintk b1;
     | Block (b1) -> found := self#locateprintk b1;
     | Switch (e,b1,_,l) -> found := (self#loceprintk e)+ self#locateprintk b1; 
     | _ -> ();
      done;
  ...
  begin
   ...
   Printf.fprintf stderr "\n+++ADDING REPORT CODE DISABLED> WARNING WARNING.\n\n+++++++++++++++++++";
	   ...
	   match cur_stmt.skind with
	    ...
	    strlist := "jiffies" :: !strlist;
			...
			if (!done_gen = -1) then (
				...
				let log_call_fundec = (emptyFunction "DPRINT" ) in
			    ...
			    Printf.fprintf stderr "WARNING RTC DISABLED.\n";
			  ...
			  false_stmt_list := list_append !false_stmt_list false_stmt; 
					    ...
					    Printf.fprintf stderr "DISABLING STATEMENT ADD.\n\n\n" ;
