...
let livelck_fns: string list =
  [ "sleep";
    ...
    "mdelay";
    "schedule_timeout";
    "wait_event_noninterruptible_timeout";
    "wait_event_interruptible_timeout";
    "wait_event_timeout";
  ];;  
(* Above functions are apart from the stuck
 * control loops.
 *)
...
(* USB functions
   dev_desc is a big problem
 *)
let usb_ip_fns: string list =
  [ "usb_string";
  ];;


(* System wear out *)
let wearout_fns: string list =
    [  "kmalloc";
       "kzalloc";
       "kmem_cache_alloc";
       "vfs_write";
...
(* Used to check for dirtyness that may impede liveness.i
 * livelocks, format string, buffer overflow, memory corruption
 * hang the system.
 *)
...
let bad_arg_functions: string list =
   [ "usb_string";
...
let lst_sec_line: int ref = ref 1;;
let lst_sec_cause: int ref = ref 0;;
let lst_fmt_bug: int ref = ref 0;;
(*
 wearout = 1
 fmt = 2
 bov = 3
...
(* Compare input string to check if its a bad function *)
   ...
   begin
     ...
     (* Check if it belongs to the incoming string list of bad
      * variables. *) 
     ...
     (* Check if it belongs to funcs. with bad return values *) 
 ...
 (* Returns the tainted argument list *)
 let isbadargument (str: string) (sl:string list): string list =
 begin
   let rc = ref [] in
 ...
 (* Compare input string to check if its a halting function *)
  ...
  (* Compare input string to check if its a wearout argument 
   Total of format string, buffer overflow, livelock, wearout
  *) 
   let isinsecure (str:string) (sl: string list) :int = 
    begin
       (* Printf.fprintf stderr "Checking isinsecure:%s.\n" str; *)
      let rc = ref 0 in
         for i = 0 to (List.length wearout_fns) - 1 do
          let wearout_string = (List.nth wearout_fns i) in
            if
              (String.compare wearout_string str = 0) then (
            rc := 1;
	    lst_sec_cause := 1
	    );
        done;
      ...
      let rc2 = ref 0 in
               for i = 0 to (List.length fmtstng_fns) - 1 do
          let fmtstng_string = (List.nth fmtstng_fns i) in
            if
              (String.compare fmtstng_string str = 0) then  (
            rc2 := 1;
	    lst_sec_cause := 2;
	    );
        done;
      ...
      let rc3 = ref 0 in
               for i = 0 to (List.length bufovflow_fns) - 1 do
          let bo_string = (List.nth bufovflow_fns i) in
            if
              (String.compare bo_string str = 0) then	(
            rc3 := 1;
 	    lst_sec_cause := 3;
	    );
        done;
      ...
      let rc4 = ref 0 in
               for i = 0 to (List.length livelck_fns) - 1 do
          let ll_string = (List.nth livelck_fns i) in
            if
              (String.compare ll_string str = 0) then	(
            rc4 := 1;
	    lst_sec_cause := 4;
	    );
        done;
...
(* This contains a list of function names and argument numbers which
 * are dirty for the given function.
 *)
let taintedfuncs : (string, int) Hashtbl.t = (Hashtbl.create 15);; 
 ...
 *)
 ...
 val mutable tainted_arg_list:  string list  =
     [
     ];

 (* Store tainted arguments to trace taint *)
 val mutable temp_bad_arguments : string list =
     [
     ];
 ...
 (* Finds all the call lvals (variables) in a CIL instruction. *)
   ...
   method isbad_exp(e:exp) : int =
    begin
        ...
        (* Printf.fprintf stderr ">>>(Pass 1) In isbad_exp	for %s(%d).\n" (exp_to_string e) ((List.length !lvalue_list) -1); *)
            for i = 0 to (List.length !lvalue_list) -1 do
                ...
                | Var (vi) ->
                        begin
			    Printf.fprintf stderr ">>>(Pass 1) In isbad_exp processing %s." vi.vname;
		...
		| Mem (e1) ->
		    begin
			(* ret_val := self#isbad_exp(e1); *)
			Printf.fprintf stderr ">>>(Pass 1) In isbad_exp(mem) processing %s.\n" (exp_to_string e1);
			(match offset with
			 | Field (f, o1) -> (Printf.fprintf stderr  ">>>(Pass 1) In isbad_exp(memf) processing %s.\n" (f.fname);
			     if (isbad f.fname temp_bad_functions = 1) then
                              begin
                                (* If tmp=*bad()*, tmp is bad *)
                                temp_bad_functions <-
                                    f.fname::temp_bad_functions;
                                 (* If tmp() { *bad()*} tmp is bad *)
                                  Hashtbl.add dirrrty
                                  (f.fname,curr_func.svar.vname) (exp_to_string
                                  e);
                                  ret_val := 1;
                              end
   ...
   (* Visits every "instruction"  and add contaminated variables, function return values
    * and arguments from possibly tainted functions. 
    *)
   ...
   begin
       let lvalue_list = ref [] in
        ...
        begin
            ...
            for i = 0 to (List.length !lvalue_list) - 1 do
                ...
                | Var  (vi) ->
                        ...
                        *)
                        if (isbad (exp_to_string e) temp_bad_functions  = 1) then
                            begin
                                temp_bad_functions <-
                                    vi.vname::temp_bad_functions;
	    ...
	    (* Add arguments instead of lvalues of tainted functions 
	    tainted_arg_list := isbadargument ((exp_to_string e) temp_bad_arguments);
            for i = 0 to (List.length !tainted_arg_list) - 1 do
		let tainted_arg = (List.nth !tainted_arg_list i) in
		Hashtbl.add dirrrty (tainted_arg, curr_func.svar.vname) (exp_to_string e);
            done;
	    *)	
        end
                        ...
                        begin
                             ...
                             if (self#isbad_exp e  = 1) then
                                     begin
                                         ...
                                         temp_bad_functions <-
                                             vi.vname::temp_bad_functions;
					 (* Record the line number when the function becomes tainted. *)
					 Printf.fprintf stderr "||Added %s(var) to temp_bad" vi.vname;
					 Hashtbl.add when_dirrrty (vi.vname, curr_func.svar.vname) loc.line;
                                     end;
	...
	(               funcs_with_cont_return
			 ...
			 Printf.fprintf stderr "Added %s(function) to tainted.\n" curr_func.svar.vname;
	);
                         DoChildren;
          end
     ...
     begin
        ...
        temp_bad_functions <- ["transfer_buffer"; "schedule";"usb_string";]; 
        block_count := 0;
...
(* Additional pass to determine tainited values passed as
 * procedure arguments upto one level of callign depth.
 *)
 
class interVisitor = object (self)
   inherit nopCilVisitor
   val mutable curr_func : fundec = emptyFunction ""; 
   val mutable curr_file : file = dummyFile;
   ...
   *  *)
   ...
   method instr_process (i: instr) : unit =
   begin
       let lvalue_list = ref [] in
        match i with
        Call(lvalue_option,e,el,loc) ->
	  ...
	  (* Part 1: Check if procedure arguments are getting tainted 
	   * 
	   * Functions      Tainted argument count
           * usb_string     2 (3rd argument)  
           * pci_read_config_word 2 
	   *)
	 
          Printf.fprintf stderr "803:Comparing %s \n\n\n" (exp_to_string e);
	  if (String.compare (exp_to_string e) "usb_string" = 0) then	(
			let cur_lval_list = ref (self#find_lvals_exp (List.nth el 2)) in
(*			Printf.fprintf stderr "List.length %d for exp %s.\n" (List.length cur_lval_list) (exp_to_string (List.nth el 2));
			Hashtbl.add dirrrty (exp_to_string (List.nth el 2), curr_func.svar.vname) (exp_to_string e);
*)
			cur_lval_list := (exp_to_string (List.nth el 2))::!cur_lval_list;
			Printf.fprintf stderr "805:Added %s %s.\n" (exp_to_string (List.nth el 2)) curr_func.svar.vname;
			for l = 0 to (List.length !cur_lval_list) - 1 do
                      	let cur_l = (List.nth !cur_lval_list l) in
			  Hashtbl.add dirrrty (cur_l, curr_func.svar.vname) (exp_to_string e);
			  Printf.fprintf stderr "805:Added %s %s.\n" cur_l curr_func.svar.vname;
			done; 
	  );


	  if (String.compare (exp_to_string e) "pci_read_config_word" = 0) then
			Hashtbl.add dirrrty ((exp_to_string (List.nth el 2)),curr_func.svar.vname) (exp_to_string e);


	  if (String.compare (exp_to_string e) "dma_map_page" = 0)|| (String.compare (exp_to_string e) "dma_map_single" = 0) 
				|| (String.compare (exp_to_string e) "pci_map_single" = 0)  then
			Hashtbl.add dirrrty ((exp_to_string (List.nth el 1)),curr_func.svar.vname) (exp_to_string e);


	  (* Part 2: Propogate taint via procedure arguments *)
	  for k = 0 to (List.length el) - 1 do
	   let cur_e = (List.nth el k) in
	   let cur_lval_list = self#find_lvals_exp cur_e in 
	     for l = 0 to (List.length cur_lval_list) - 1 do
	       let cur_l = (List.nth cur_lval_list l) in   
	       (* Printf.fprintf stderr "Argument:%d: %s.\n"l (exp_to_string cur_e); *)
	       try
	         let ret_str = (Hashtbl.find dirrrty ((cur_l), curr_func.svar.vname)) in
	         Printf.fprintf stderr "*****>>>>>>>tainted arg %s(%d) in procedure call %s.\n" cur_l l (exp_to_string e);
		 Hashtbl.add taintedfuncs (exp_to_string e) l;
		 (* This is bugged up 
		 let fvarinfo = findOrCreateFunc curr_file (exp_to_string e)  in
		 match fvarinfo.vtype with
		   TFun (ty, _, _, _) -> Printf.fprintf stderr "*";
		   |_ -> (); 
		*)
	       with Not_found -> ();
		 done;			 
    ...
    (* Visits every function (inter procedural pass) *)
     method vfunc (f: fundec) : fundec visitAction =
     begin
	 curr_func <- f; 
         DoChildren;
     ...
     (* Start the visiting *)
     curr_file <-f ;
     Printf.fprintf stderr "Detecting interprocedural stuff.\n"; 
     visitCilFileSameGlobals (self :> cilVisitor) f;
     (* Here match all fn names from global fn list and insert in dirrrty 
     Hashtbl.iter taintedfuncs self#findtaintprocargs;	*)
     iterGlobals f (fun glob -> match glob with 

       GFun(f,_) ->(
		 (* Printf.fprintf stderr "Fn is %s.\n" f.svar.vname; *)
		try
		let ret_str = (Hashtbl.find taintedfuncs (f.svar.vname)) in
	        (Printf.fprintf stderr "ret_str is %d.\n"  ret_str);
		if (ret_str < (List.length  f.sformals)) then 
			let cur_arg = (List.nth f.sformals ret_str) in
		Hashtbl.add dirrrty (cur_arg.vname, f.svar.vname) f.svar.vname;			
		with Not_found -> ();
...
class driverVisitor = object (self) (* self is equivalent to this in Java/C++ *)
    ...
    val mutable async_ticks_ctr: int = 0;
    ...
    val mutable sec_count = 0;
    val mutable wo_count = 0;
    val mutable fmt_count = 0;
    val mutable dma_count = 0;
    val mutable ll_count  = 0;
    val mutable bo_count = 0;
   ...
   *  *)
   ...
   begin
	...
	begin 
     ...
     | Const(_) -> (Hashtbl.add hist_array_dirty (!array_mask_set, curr_func.svar.vname) "yes"; [];)              
     | _ -> [];
   end
 ...
 (* Finds all the array lvals in a CIL instruction *)
  ...
  method locate_dupctrs_in_block (b: block)(s: string list) : string list =
   ...
   method locate_asynch_in_block (b:block) :int = 
   begin
	let rc = ref 0 in
	for i = 0 to (List.length b.bstmts) - 1 do
          let cur_stmt = (List.nth b.bstmts i) in
		 ...
		 let str_list = self#find_lvals_exp e in
		   ...
		   if (String.compare (cur_str) "schedule" = 0)  then
		     (rc:=1;);
		   done;
		 end
		 |_ ->();
...
(*	    | Return (_,_) -> rc_list := "__nooks_timer"::!rc_list; *)
	...
	*)
    ...
    | Instr (ilist) ->
         ...
         match cur_i with
              | Call (_,e, el, loc) -> Printf.fprintf stderr "lckpk %s" (exp_to_string e);if ((String.compare (exp_to_string e) "printk") = 0) then found:= 1;
	      | _ -> ();
	done;
		...
		|  Set (lv, e, loc) ->
		  begin
                  ...
                  | Const (con) -> (
                        ...
                        match con with
                        | CInt64 (i, ik, Some s) -> ( if (i < (Int64.of_int 0)) then ( goto_label := str;  Printf.fprintf stderr "RET INCR for %s " (exp_to_string e) ; ret_seen <- ret_seen + 1; pk_count <- pk_count + self#locateprintk b; self#addreportcode b;))
                        | CInt64 (i, ik, None) -> ( if (i < (Int64.of_int 0)) then (self#addreportcode b; goto_label := str;ret_seen <- ret_seen + 1; pk_count <- pk_count + self#locateprintk b;Printf.fprintf stderr "NRET INCR for %s" (exp_to_string e); self#addreportcode b;));
...
|	  Instr(ilist) ->
	  begin
                ...
                for j = 0 to (List.length ilist) - 1 do
                  let cur_instr = (List.nth ilist j) in
                    match cur_instr with
                    | Call(lvalue_option,e,el,loc) ->
                          if (isinsecure (exp_to_string e) [] > 0)  then
                            begin
                                 for k = 0 to (List.length el) - 1 do
                                   let cur_e = (List.nth el k) in
                                   let cur_lval_list = ref (self#find_lvals_exp cur_e) in
				   cur_lval_list := exp_to_string(cur_e)::!cur_lval_list; 
				   Printf.fprintf stderr "1998:Checkign for %s lval_list_len %d.\n" (exp_to_string cur_e) (List.length !cur_lval_list); 
 				     for l = 0 to (List.length !cur_lval_list) - 1 do
                                      let cur_l = (List.nth !cur_lval_list l) in   
                                        Printf.fprintf stderr "1993: Arg: %s.\n" (exp_to_string cur_e); 
				       try
					 (* Check for %s and %n for format string.
				          let match_regexp = regexp (".*"^"%s"^".*") in
				          if (Str.string_match match_regexp (exp_to_string cur_e) 0) = true  then (	
					   lst_fmt_bug :=1;
  					  );
				          *)
					  let match_regexp = regexp (".*"^"%n"^".*") in
				          if (Str.string_match match_regexp (exp_to_string cur_e) 0) = true  then (
					   lst_fmt_bug :=1;
					  );
					 ...
					 let ret_str = (Hashtbl.find dirrrty ((cur_l), curr_func.svar.vname)) in
                                         Printf.fprintf stderr ">>>>>>>Tainted arg %s for fn %s.\n*********\n " cur_l (exp_to_string e);
					 if (!lst_sec_line != loc.line) then 	(
					   lst_sec_line := loc.line;
					   (* error_line_nos := !error_line_nos@[(Printf.sprintf "sec:%d"loc.line)]; *)
				 	   sec_count <- 1  + sec_count;
					   if (!lst_sec_cause = 1) then ( 
					     wo_count <- wo_count + 1;
					     error_line_nos := !error_line_nos@[(Printf.sprintf "wo:%d"loc.line)];
					   );
					   if (!lst_sec_cause = 2) then (
					     
					     (* If tainted argument count is greater than fisrt and all we see is %d, %x then
                                                probably this is not a vlnerability.*)
					     if (k = 0)||(!lst_fmt_bug = 1) then (
					       fmt_count <- fmt_count + 1;
					       error_line_nos := !error_line_nos@[(Printf.sprintf "fmt:%d:%d"loc.line k)];
					       lst_fmt_bug :=0;
					     );
					      
				           );
					   if (!lst_sec_cause = 3) then  (
					     bo_count <- bo_count + 1; 
					     error_line_nos := !error_line_nos@[(Printf.sprintf "bo:%d"loc.line)];
                                           );
					   if (!lst_sec_cause = 4) then  (		
					     ll_count <- ll_count + 1;
					     error_line_nos := !error_line_nos@[(Printf.sprintf "ll:%d"loc.line)];
					   );
					   if (!lst_sec_cause = 0) then (
					     dma_count <- dma_count + 1;
  					     error_line_nos := !error_line_nos@[(Printf.sprintf "dma:%d"loc.line)];
					   );
					   lst_sec_cause := 0; 
					 );
				      with Not_found -> ();
     ...
     | Loop(b,ln,Some labelstmt,Some breakstmt) ->
	   ...
	   (* Special case for USB. If schedule is called in an infinite loop *)
	    if ((!done_gen != -1) && (self#locate_asynch_in_block b = 1)) then 
		(async_ticks_ctr <- async_ticks_ctr + 1;
		error_line_nos := !error_line_nos@[(Printf.sprintf "async-tics:%d"ln.line)];
		);
			...
			if (!done_gen = -1) then (
				...
				let rtc_pk = self#locateprintk b in	
 			  ...
 			  pk_in_rtc <- self#locateprintk b;  
                          let false_stmt_old = (mkStmt(Break(locUnknown))) in
		...
		for m = 0 to (List.length !strlist) - 1 do
	          let cond_str = (List.nth !strlist m) in
          		for n = 0 to (List.length counters_in_loop) -1  do
                 	let ctr_str = (List.nth counters_in_loop n) in 
	        	if (String.compare cond_str ctr_str = 0) then (
		        Printf.fprintf stderr "Counter exp detected. \n";
		        found := 1;
...
(*	Printf.fprintf stderr "incoming exp is %s.\n" incoming_exp; *)
	...
	with Not_found -> (
        ...
        *)  (* checks each and every variable is dirty in the expression. *)
		...
		(try ( 
			...
			Printf.fprintf stderr "BUG:^^Memory op found for exp %s %s.\n" (exp_to_string e) ret_str; 
			unsafe_dref := 1;
			);
   ...
   begin
      let retval = ref 0 in
      (* Printf.fprintf stderr ">>>>>HERE find_deref_in_exp: %s \n\n\n" (exp_to_string e); *)
      match e with  
      |   Lval (lval) -> (
           match lval with (lhost, offset) -> (
               match lhost with
   ...
   begin
    ...
    method top_level (f:file) :unit =
      begin
        ...
        Printf.fprintf stderr "The count of security bugs is %d.\n" (List.length
        per_fun - 1 + async_ticks_ctr);
	...
	(*	
	Printf.printf " mem bugs %d hlt %d ret %d rtc %d pk %d pkrtc %d." mem_deref_bugs !halt_count return_on_device_error report_timeout_counter ret_pk_count pk_in_rtc; (*num_bad_ptr_lvals; *)
	Printf.fprintf stderr " m b %d hlt %d ret %d rtc %d pk %d pkrtc %d." mem_deref_bugs !halt_count return_on_device_error report_timeout_counter ret_pk_count pk_in_rtc; (* num_bad_ptr_lvals; *)
	
	*)

	if (mem_deref_bugs + !halt_count + fmt_count + bo_count + sec_count + ll_count + wo_count) > 0 then (	
	Printf.printf " mem bugs %d hlt %d fmt %d bo %d sec %d ll %d wo %d async %d." mem_deref_bugs !halt_count fmt_count bo_count sec_count ll_count wo_count async_ticks_ctr; (*num_bad_ptr_lvals; *)
	Printf.fprintf stderr " m b %d hlt %d fmt %d bo %d sec %d ll %d wo %d async %d." mem_deref_bugs !halt_count fmt_count bo_count sec_count ll_count wo_count async_ticks_ctr; (* num_bad_ptr_lvals; *)
...
let dobeefyanalysis (f:file)  : unit = 	
  begin
      ...
      *)
      ...
      let proVisitor : interVisitor = new interVisitor in
      proVisitor#top_level f;
