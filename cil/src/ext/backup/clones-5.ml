   ...
   begin
     ...
     match s.skind with 
       | Instr(ilist) -> ( (* Hashtbl.add stmt_cluster !cur_l 1; *)
                           for j = 0 to (List.length ilist) - 1 do
                             let cur_instr = (List.nth ilist j) in
                               match cur_instr with   
                                    | Call(lvalue_option,e,el,loc) -> (Hashtbl.add stmt_cluster loc.line 5;);
                       ...
                       DoChildren;);
       | Return(_, loc) -> Hashtbl.add stmt_cluster !cur_l 30; DoChildren;
       | Goto (_, loc) -> Hashtbl.add stmt_cluster !cur_l 25; DoChildren;
       | Break (loc) -> Hashtbl.add stmt_cluster !cur_l 20; DoChildren;
       | Continue (loc) -> Hashtbl.add stmt_cluster !cur_l 35; DoChildren;
       | If (_, _, _, loc) -> Hashtbl.add stmt_cluster !cur_l 40; DoChildren;
       | Switch (_, _, _, loc) -> Hashtbl.add stmt_cluster !cur_l 50 ; DoChildren;
       | Loop (_,loc, _, _) -> Hashtbl.add stmt_cluster !cur_l 10; DoChildren;
       | Block (_) -> Hashtbl.add stmt_cluster !cur_l 1; DoChildren;
       | TryFinally (_, _, loc) -> Hashtbl.add stmt_cluster !cur_l 55; DoChildren;
       | TryExcept (_, _, _, loc) -> Hashtbl.add stmt_cluster  !cur_l 60; DoChildren;                   
       | _ ->DoChildren; 
